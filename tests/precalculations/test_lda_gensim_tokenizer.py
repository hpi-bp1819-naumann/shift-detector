...
class TestLdaGensimTokenizer(unittest.TestCase):
    ...
    def setUp(self):
        self.token1 = LdaGensimTokenizer(columns=['col1'], stop_words='english')
        self.token2 = LdaGensimTokenizer(columns=['col1'], stop_words='english')
        self.token3 = LdaGensimTokenizer(columns=['col1'], stop_words='french')
    ...
    def test_exception_for_columns(self):
        self.assertRaises(TypeError, lambda: LdaGensimTokenizer())
        self.assertRaises(TypeError, lambda: LdaGensimTokenizer(columns=None))
        self.assertRaises(TypeError, lambda: LdaGensimTokenizer(columns=42))

    def test_exception_for_stop_words(self):
        self.assertRaises(ValueError, lambda: LdaGensimTokenizer(columns=['col1'], stop_words=['english', 'abcd']))
        self.assertRaises(ValueError, lambda: LdaGensimTokenizer(columns=[''], stop_words='abcd'))
        self.assertRaises(ValueError, lambda: LdaGensimTokenizer(columns=[''], stop_words=['english', ' abcd']))
        self.assertRaises(TypeError, lambda: LdaGensimTokenizer(columns=[''], stop_words=['english', 42]))
