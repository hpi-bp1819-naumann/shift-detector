...
class TestSimpleCheck(unittest.TestCase):
    ...
    def test_quartile_metrics(self):
        numerical_df_1 = pd.DataFrame([[0], [1], [2], [3], [4]],
                                      columns=['col_1'])
    ...
    def test_categorical_valies(self):
        comparison_categorical = self.check.run()['categorical_comparison']

        self.assertEqual(comparison_categorical[0]['blue']['df1'], 1/3)
        self.assertEqual(comparison_categorical[0]['green']['df1'], 0.5)
        self.assertEqual(comparison_categorical[0]['green']['df2'], 0.5)
        self.assertEqual(comparison_categorical[0]['red']['df1'], 1/6)
        self.assertEqual(comparison_categorical[0]['red']['df2'], 0.5)
